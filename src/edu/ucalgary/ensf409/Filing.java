...
/**
 * Class for the furniture category of Filing this accounts for the cheapestPrice
...
public class Filing {
    ...
    /**
     * Category for the Filing which is the filing
     */
    private String category;

    /**
     * The type which includes small, medium and large
     */
    private String type;
    ...
    //Default Filing constructor:
    Filing() {
        //Does nothing
    }

  /**
   * Filing constructor takes in 6 parameters for creating the Filing and intializing it 
   */
    Filing(String category, String type, int quantity, String dburl, String username, String password) {
        this.category = category;
        this.type = type;
        this.quantity = quantity;
        this.DBURL = dburl;
    ...
    public void getEverything(String category) {
        ...
        try {
            ...
            ResultSet rs = stmnt.executeQuery("SELECT * FROM FILING WHERE Type = " + "'" + category + "'");
    ...
    public void getManufacturers(String category) {
        ...
        try {
            ...
            stmnt = createConnection.createStatement();
            //Uses the result set to gather data about the filing which is for the specfic
     ...
     * for example a small filing. Then it will just add 0s in the array list. 
     ...
     * combination which was found from the select filing table. 
     */
    public void findingCombinations() {
        ...
        //The amount of each furniture piece to make one filing
        int numOfRails = 0;
        int numOfCabinets = 0;
        int numOfDrawers = 0;
        ...
        for (int a = 0; a < possibleCombinations.size(); a++) {
            ...
            for (int i = 0; i < partIDs.size(); i++) {
                ...
                try {
                    ...
                    while(rs2.next()) {
                         //Checks for the filings rails and what their status 
                        //Is in the database 
                        if (rs2.getString("Rails").equals("Y")) {
                            numOfRails++;
                        }
                         //Checks for the filings drawers and what their status 
                        //Is in the database 
                        if (rs2.getString("Drawers").equals("Y")) {
                            numOfDrawers++;
                        }
                         //Checks for the filings cabinets and what their status 
                        //Is in the database 
            ...
            if (numOfRails >= maxParts && numOfDrawers >= maxParts && numOfCabinets >= maxParts ) {
    ...
    public void removeParts() {
        ...
        try {
            ...
            for (int i = 0; i < removedParts.size(); i++) {
                ...
                stmnt.executeUpdate("DELETE FROM FILING WHERE ID = "+ "'" + removedParts.get(i) + "'");
                stmnt.close();
            }
        }
